...
function findBiggestPublisher(arr) {
  ...
  for (const key of Object.entries(emptyObj)) {
    console.log(key);
    const publisherNumArr = Object.values(emptyObj);
    higherNumber = publisherNumArr[0];
    lowerNumber = publisherNumArr[0];
    for (let i = 1; i < publisherNumArr.length; i++) {
      if (publisherNumArr[i] > higherNumber) {
        higherNumber = publisherNumArr[i];
        highName = key[i];
      } else if (publisherNumArr[i] < lowerNumber) {
        lowerNumber = publisherNumArr[i];
        lowName = key;
      }
    }
  }
